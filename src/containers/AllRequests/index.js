...
let SearchRequests = () => {
    ...
    useEffect(async () => {
        let arr = [];
        const q = query(collection(db, "requests"), where("cnic", "==", search));
            ...
            <Container fluid className='aboutMain'>
                <Row>
                    <Col xs={{ span: 12, order: "last" }} sm={{ span: 12, order: "last" }} md={{ span: 12, order: "first" }} lg={{ span: 12, order: "first" }} className="headingMain">
                        <h1 className="heading1">Search Requests</h1>
                        <MyInputText type="number" fullWidth={true} label="Search" name="search" onChange={(e) => setSearch(e.target.value)} value={search} />
